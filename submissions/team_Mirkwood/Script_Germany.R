...
df_in <-  read_csv('data/Germany/input_data.csv') %>% 
  rename(Date = time) %>% 
  ...
  mutate(pp_agg7 = rollmeanr(pp, 7, fill = NA)) %>%
  mutate(pp_agg30 = rollmeanr(pp, 30, fill = NA)) %>%
  mutate(pp_agg365 = rollmeanr(pp, 365, fill = NA)) %>%
  mutate(hu_agg7 = rollmeanr(hu, 7, fill = NA)) %>%
  mutate(hu_agg30 = rollmeanr(hu, 30, fill = NA)) %>%
  mutate(hu_agg365 = rollmeanr(hu, 365, fill = NA)) %>%
  mutate(fg_agg7 = rollmeanr(fg, 7, fill = NA)) %>%
  mutate(fg_agg30 = rollmeanr(fg, 30, fill = NA)) %>%
  mutate(fg_agg365 = rollmeanr(fg, 365, fill = NA)) %>%
  mutate(qq_agg7 = rollmeanr(qq, 7, fill = NA)) %>%
  mutate(qq_agg30 = rollmeanr(qq, 30, fill = NA)) %>%
  mutate(qq_agg365 = rollmeanr(qq, 365, fill = NA))

#####Read head data and joined to predictor data

df_head <-  read_csv('data/Germany/heads.csv')
...
df_train <- df %>% 
  filter(between(Date, date('2002-05-01'), date('2016-12-31')))

df_test <- df %>% 
  filter(between(Date, date('2017-01-01'), date('2021-12-31')))
...
write_rds(df_stoch, 'submissions/team_Mirkwood/df_stoch_Germany.rds')

end_time <- Sys.time()

time_unc <- end_time - start_time

df_stoch <- read_rds('submissions/team_Mirkwood/df_stoch_Germany.rds')
...
submission_csv <- read_csv('submissions/team_Mirkwood/submission_form_Germany.csv') %>% 
  dplyr::select(Date)
...
left_join(submission_csv, df_out) %>% 
  write_csv('submissions/team_Mirkwood/submission_form_Germany_completed.csv')
